...
class Session(object):
    ...
    def __clear_data(self):
        ...
        if self.sid:
            ...
            client.delete(self.sid, namespace='')  # not really needed; it'll go away on its own
            try:
    ...
    def __retrieve_data(self):
        ...
        client = memcache.Client()
        pdump = client.get(self.sid, namespace='')
        if pdump is None:
    ...
    def save(self, persist_even_if_using_cookie=False):
        ...
        client = memcache.Client()
        client.set(self.sid, pdump, namespace='', time=self.get_expiration())  # may fail if memcache is down
