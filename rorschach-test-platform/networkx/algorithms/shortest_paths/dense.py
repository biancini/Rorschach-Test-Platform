...
def floyd_warshall_numpy(G, nodelist=None, weight='weight', dtype=None, infvalue=None):
    ...
    infvalue : infinite data value, optional
        The maximin value for dtype type must be specificied to 
        represent the infitite value (for nodes not connected one
        with the other)
        If None, then the NumPy inf is used.
    ...
    n,m = A.shape
    I=np.identity(n)
    if infvalue == None: infvalue = np.inf
    A[A==0]=infvalue # set zero entries to inf
    A[I==1]=0 # except diagonal which should be zero
    for i in range(n):
        r = A[i,:]
        r1 = r + r.T
        r1[r1<0]=infvalue # fix for int type, the sum silentry overflows
        A = np.minimum(A, r1)
    return A
