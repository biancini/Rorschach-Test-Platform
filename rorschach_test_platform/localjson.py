...
class JsonReader(object):
    ...
    def _read(self):
        ...
        peek = self._peek()
        if peek is None:
            raise ReadException, "Nothing to read: '%s'" % self._generator.allchars()
        if peek == '{':
            ...
            raise ReadException, "Input is not valid JSON: '%s'" % self._generator.allchars()
    ...
    def _assertNext(self, ch, target):
        if self._next() != ch:
            raise ReadException, "Trying to read %s: '%s'" % (target, self._generator.allchars())
            ...
            while self._peek() != '"':
                ...
                if ch == "\\":
                    ...
                    if ch in 'brnft':
                        ...
                        raise ReadException, "Not a valid escaped JSON character: '%s' in %s" % (ch, self._generator.allchars())
                result = result + ch
        except StopIteration:
            raise ReadException, "Not a valid JSON string: '%s'" % self._generator.allchars()
        nn = self._next()
        assert nn == '"'
        return result
    ...
    def _readComment(self):
        ...
        if second == "/":
            ...
            raise ReadException, "Not a valid JSON comment: %s" % self._generator.allchars()
        ...
        try:
            ...
            while not done:
                ...
                if not done and ch == "/" and self._peek() == "*":
                    raise ReadException, "Not a valid JSON comment: %s, '/*' cannot be embedded in the comment." % self._generator.allchars()
            self._next()
        except StopIteration:
            raise ReadException, "Not a valid JSON comment: %s, expected */" % self._generator.allchars()
    ...
    def _readArray(self):
        ...
        while not done:
            ...
            if not done:
                ch = self._next()
                if ch != ",":
                    raise ReadException, "Not a valid JSON array: '%s' due to: '%s'" % (self._generator.allchars(), ch)
        nn = self._next()
        assert ']' == nn
        return result
    ...
    def _readObject(self):
        ...
        while not done:
            ...
            ch = self._next()
            if ch != ":":
                raise ReadException, "Not a valid JSON object: '%s' due to: '%s'" % (self._generator.allchars(), ch)
            self._eatWhitespace()
            val = self._read()
            result[key] = val
            self._eatWhitespace()
    ...
    def _next(self):
        return self._generator.nextchar()
